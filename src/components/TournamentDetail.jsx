...
function TournamentDetail() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        if (
          tournamentResponse.data.teams &&
          tournamentResponse.data.teams.length > 0
        ) {
        ...
        const pointsTableResponse = await api.get(
          `/api/tournaments/${tournamentId}/points-table`
        );
        setPointsTable(pointsTableResponse.data);

        const keyPlayersResponse = await api.get(
          `/api/tournaments/${tournamentId}/key-players`
        );
  ...
  const getAllMatches = () => {
    ...
    const leagueMatches = (tournament.matches || []).map((match) => ({
      ...match,
      matchType: match.matchType || "League",
    }));
    const semiFinalMatches = (tournament.semiFinals || []).map((match) => ({
      ...match,
      matchType: match.matchType || "Semi-Final",
    }));
    const finalMatch = tournament.final
      ? [
          {
            ...tournament.final,
            matchType: tournament.final.matchType || "Final",
          },
        ]
      : [];
  ...
  if (!tournament) {
    return (
      <motion.div
        ...
        style={{ minHeight: "100vh", color: "var(--text-color)" }}
      >
          ...
          {tournament.winner && (
            ...
            >
              ...
              <p
                ...
                style={{ color: "var(--badge-success-bg)" }}
              >
                        ...
                        {row.map((match) => {
                          ...
                          const team2 =
                            match.teams && match.teams[1]
                              ? match.teams[1]
                              : { name: "Unknown" };
                          const team1Score = formatScore(
                            match.runsScored?.innings1,
                            match.oversBowled?.innings1,
                            match.wickets?.innings1
                          );
                          const team2Score = formatScore(
                            match.runsScored?.innings2,
                            match.oversBowled?.innings2,
                            match.wickets?.innings2
                          );
                          const result =
                          ...
                          return (
                            ...
                            >
                              ...
                              >
                                <div
                                  className="match-card p-4 rounded-3 shadow-lg"
                                  style={{
                                    background: "var(--card-bg-gradient)",
                                  }}
                                >
                                  <p
                                    className="small mb-2 fw-bold"
                                  ...
                                  <div className="d-flex justify-content-between align-items-center flex-wrap">
                                    <div className="d-flex align-items-center flex-wrap">
                                      ...
                                      <span className="text-muted fw-bold mx-2 mb-2">
                                        VS
                                      </span>
                        ...
                        <div className="d-flex align-items-center">
                          ...
                          <div>
                            ...
                            <p className="mb-0 small">
                              {players.filter((p) => p.team === team.name)
                                .length || 0}{" "}
                              Players
                            </p>
                          </div>
                    ...
                    <>
                      <div className="d-flex align-items-center mb-4">
                        ...
                        <p className="mb-0 ms-3 text-muted fs-5">
                          {players.filter((p) => p.team === selectedTeam.name)
                            .length || 0}{" "}
                          Players
                        </p>
                      </div>
                      ...
                      {Object.entries(
                        groupPlayersByRole(selectedTeam.name)
                      ).map(
                        ([role, players]) =>
                          players.length > 0 && (
                            <motion.div
                          ...
                          >
                            ...
                            <td
                              style={{
                                color:
                                  entry.nrr >= 0
                                    ? "var(--badge-success-bg, #28a745)"
                                    : "var(--badge-warning-bg, #dc3545)",
                              }}
                            >
              ...
              {!keyPlayers ? (
                ...
                <div className="row">
                  {[
                    {
                      title: "Most Runs",
                      data: keyPlayers.topRunScorers,
                      valueKey: "runs",
                      suffix: "runs",
                    },
                    {
                      title: "Most Wickets",
                      data: keyPlayers.topWicketTakers,
                      valueKey: "wickets",
                      suffix: "wickets",
                    },
                    {
                      title: "Most Sixes",
                      data: keyPlayers.topSixes,
                      valueKey: "sixes",
                      suffix: "sixes",
                    },
                    {
                      title: "Most Fours",
                      data: keyPlayers.topFours,
                      valueKey: "fours",
                      suffix: "fours",
                    },
                    {
                      title: "Highest Strike Rate",
                      data: keyPlayers.topStrikeRate,
                      valueKey: "strikeRate",
                      format: (v) => v.toFixed(2),
                    },
                    {
                      title: "Best Economy",
                      data: keyPlayers.bestEconomy,
                      valueKey: "economy",
                      format: (v) => v.toFixed(2),
                    },
                  ].map((section, index) => (
                    ...
                    >
                      ...
                      >
                        ...
                        {section.data && section.data.length > 0 ? (
                          <>
                            {section.data
                              .slice(0, visiblePlayers[section.title] || 5)
                              .map((player, idx) => (
                                <motion.div
                                  key={idx}
                                  initial={{ opacity: 0 }}
                                ...
                                >
                                  ...
                                  >
                                    {section.format
                                      ? section.format(player[section.valueKey])
                                      : `${player[section.valueKey]} ${
                                          section.suffix || ""
                                        }`}
                                  </span>
                                </motion.div>
                            ...
                            {section.data.length >
                              (visiblePlayers[section.title] || 5) && (
                              <motion.div
                                initial={{ opacity: 0 }}
                                animate={{ opacity: 1 }}
                                transition={{ delay: 0.3 }}
                              ...
                              >
                                <motion.button
                                  ...
                                  onClick={() =>
                                    handleLoadMorePlayers(section.title)
                                  }
                                >
                                  Load More
                                </motion.button>
